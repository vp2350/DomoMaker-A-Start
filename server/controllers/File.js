...
const retrieveFilesByFileName = (req, res) => {
  console.dir(req.query);
  if (!req.query.fileSearchName) {
    return res.status(400).json({
      error: 'Missing User Name!',
    });
  }

  return filedb.FileModel.findByName(req.query.fileSearchName, (error, doc) => {
    if (error) {
      console.dir(error);
      return res.status(400).json({
        error: 'An error occured retrieving the files. ',
...
module.exports.retrieveFilesByFileName = retrieveFilesByFileName;
